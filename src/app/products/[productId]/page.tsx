...
// type Tproduct={name:string,description:string,catalog:Icatalog,price:{oldest:},oldPrice:number,categories:{name:string,id:string}[]}

export default async function Product({params}:{params:{productId:string}}){ 
   
        const p=new Products();
        const h=await p.getOne({length:6,filter:{codeBar:params?.productId}});
      
        const {name,description,catalog,price,category}:Products=await p.getOne({length:6,filter:{codeBar:params?.productId}});
        const {oldPrice,currentPrice}={oldPrice:price.oldest?.current,currentPrice:price.current};
         console.log({name,description,catalog,currentPrice,oldPrice,category,status:p.checkStatus()})
        return <>
            <div  className="pg-5">
              <section>
                    <div></div>
              ...
              <section> 
                ...
                <div>
                    ...
                    <div className="scrolling">
                          ...
                          {/* { category.map((x)=><button className="btn-active-ground-w" key={x.id} value={x.id}>{x.name}</button>)} */}
                    </div> 
                    <section>
