...
type TproductShow={id:string,name:string,imgProduct:string,price:number,oldPrice:number}
export function ProductShow({id,name,imgProduct,price,oldPrice}:TproductShow){
    return <>
        <div className="show-p" >
            <Link href={`/products/${id}`}> 
                   <img  alt="pro" src={imgProduct} />
                   <div>
                        <p>{name}</p> 
                        <h2>{price}<sub> Da</sub></h2> 
                        {oldPrice && <span id="second">{oldPrice} Da</span>}
                        <span id="buy">
